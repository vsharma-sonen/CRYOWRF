...
//     cout << "C API, creating a new object" << endl;
     FOO* kkk = new SnowpackInterface[in_size];
     double* ttt = new double;
//     cout << kkk << "\t" << sizeof(ttt) << endl;
     return kkk ; // new SnowpackInterface [in_size];
}

FOO* allot_add_foo(FOO* in_type, int loc){
//     cout << in_type << "\t" << loc << "\t" << in_type+loc << endl;
     return in_type+loc;
}

int init_data_foo(FOO* foo,int snpack_layers_to_save,double Lat, double Lon, double Altitude, double sn_tsk, double in_calc_step_length, 
    ...
    return foo->init_sn(snpack_layers_to_save,Lat,Lon,Altitude,sn_tsk,in_calc_step_length,f_counter, in_grid, I, J,yr,month,day,hour,minute,
...
int run_foo(FOO* foo,int xxx,double h_of_met_vals,double l_TA,double l_RH, double l_VW, 
...
//     cout << "C API, run the model" << endl;
     return foo->nextStep(xxx,h_of_met_vals,l_TA,l_RH,l_VW,l_VW_MAX,l_DW,l_drift,l_conc,l_csalt,l_q_lb,l_N_lb,l_iswr,
