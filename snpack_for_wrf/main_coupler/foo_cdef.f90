interface
    function create_foo_c(in_size) bind(C, name="create_foo")
        use iso_c_binding
        implicit none
    ...
    function allot_add_foo_c(in_pointer,loc) bind(C, name="allot_add_foo")
        use iso_c_binding
        implicit none
        type(c_ptr)           :: allot_add_foo_c
        type(c_ptr),value     :: in_pointer
        integer(c_int), value :: loc
    end function
 
    function init_data_foo_c(foo,snpack_layers_to_save,a,b,c,d,e,f,g,i,j,yr,month,day,hour,minute,snpack_nlayers, &
&                            arr_T,arr_thick,arr_volI,arr_volW, &
&                            arr_volV,arr_rg,arr_rb,arr_dd,arr_sp,arr_cdot,arr_meta,&
&                            arr_depd,start_from_file,wrf_rho,snowh,snow,snpack_dt,snpack_write_dt ) bind(C, name="init_data_foo")
        use iso_c_binding
        implicit none
        real(c_double) :: init_data_foo_c
    ...
    function run_foo_c(foo,snpack_layers_to_save,a1,a2,a3,a4,a5,a6,d1,d2,d3,d4, &
...
&                       wrf_rho,bs_bdg_total,qi_in,qni_in,bs_K,bs_mass_turb,bs_number_turb,in_hsalt,psi_s) bind(C, name="run_foo")
        use iso_c_binding
        implicit none
        integer(c_int),value :: snpack_layers_to_save
