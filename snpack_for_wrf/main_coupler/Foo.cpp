		...
		if (start_from_file) {
		  ...
		  int nLayers = std::min(snpack_nlayers,loc_snpack_lay_to_sav);
		      ...
		      vecSSdata.Ldata[ll].phiVoids = arr_volV[nLayers-1-ll]; 
                      vecSSdata.Ldata[ll].phiSoil  = 0; 
                      vecSSdata.Ldata[ll].SoilRho  = 0;
                      vecSSdata.Ldata[ll].SoilK    = 0; 
                      vecSSdata.Ldata[ll].SoilC    = 0; 
		      ...
		      if( (I==1) && (J==31)) {
                      std::cout << "INIT SNPACK:\t" << vecSSdata.Ldata[ll].hl  << ","
                                << vecSSdata.Ldata[ll].tl     << ","
                                << vecSSdata.Ldata[ll].phiIce   << ","
                                << vecSSdata.Ldata[ll].phiVoids   << std::endl; }
